            ...
            if (pollRequest.isWatchList() && user != null) {
                synchronized (state) {
                    ...
                    for (WatchListState newState : newStates) {
						try{
							String value = newState.getValue(); //gets the value of the data point from newState 
																//and puts it into a string
							double myDouble = Double.valueOf(value).doubleValue(); //converts value into a double to be 
																				   //used with the pow variable
							double pow = Math.pow(10,2); //gives the power of 2 (10^2)
							myDouble = myDouble * pow; //multiply myDouble by pow to move the decimal point 
													   //over by 2 places to the right
							myDouble = Math.floor(myDouble); //rounds to the closest integer to myDouble
							myDouble = myDouble / pow; //puts the value back in its original state by 
													   //moving the deimcal point over two places to the left
							value = String.valueOf(myDouble); //converts myDouble to a string and puts 
															  //the value of myDouble into the string variable value
							newState.setValue(value); //sets the value of newState to value, making it truncated
						}catch(Exception e){} //if try doesn't work, catches and throws exceptions
